...
**Dashboard/Stats merge and Learning page rework (2026-07-07)**, direct follow-up feedback after M4/M5:
- Removed the dedicated Statistics page (`/stats`) — its Weekly/Monthly/Yearly toggle and trend chart are now embedded directly on the Dashboard, replacing the old fixed-to-last-7-days "Weekly hours" chart. One fewer nav item; the range still lives in the URL (`?range=month`).
- Favoriting is no longer root-only: any item, at any depth, can be favorited (`SetFavorite`'s old rejection removed). A favorited non-root item shows in the Favs tab as its own standalone entry — itself plus its own descendants, with its real ancestors/siblings excluded — not nested inside the whole original topic. See ADR-030.
